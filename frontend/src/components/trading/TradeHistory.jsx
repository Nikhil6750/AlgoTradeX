...
function ScoreBadge({ score, riskLevel }) {
    ...
    return (
        <div className="inline-flex items-center gap-1 rounded-full border border-[#222222] bg-[#111111] px-2 py-0.5 text-[10px] font-bold text-white">
            <span>{pct}%</span>
            <span className="opacity-50">·</span>
            <span>{riskLevel || "Scored"}</span>
        </div>
    );
}
                            ...
                            {trades.map((t, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {/* Direction */}
                                        <td className="px-2 py-3 text-center font-bold text-[10px] tracking-widest uppercase text-white">
                                            {t.type}
                                        </td>
                                        {/* Entry $ */}
                                        ...
                                        <td className="px-4 py-3 text-right font-mono text-xs font-semibold text-white">
                                            {(t.pnl ?? 0) > 0 ? "+" : ""}{((t.pnl ?? 0) * 100).toFixed(2)}%
                                        </td>
                                        {/* AI Score */}
                                        {hasScores && (
